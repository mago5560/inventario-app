...
@Component({
  ...
  templateUrl: './productodescuento.component.html',
  styleUrls: ['./productodescuento.component.scss']
})
export class ProductodescuentoComponent implements OnInit {

  
  public _datos:ProductoDescuento[]=[];
  public _descripcion:string="";

  constructor(private service:ProductoDescuentoService,private router: Router,public rutaActiva: ActivatedRoute,private toastr: ToastrService,private modalService: NgbModal) { }
  ...
  modificar(data:ProductoDescuento){
    this.router.navigate(['./tecleo',data.idProductoDescuento], { relativeTo: this.rutaActiva });
  }

  
  openDialog(modalEliminar,data:ProductoDescuento) {
    this.modalService.open(modalEliminar, {ariaLabelledBy: 'modal-basic-title'}).result.then((result) => {
      //Aceptar
      this.service.eliminar(data.idProductoDescuento).subscribe(data => {       
